...
public class BlackySurfaceWorldData: BlackyWorldDataMap<ushort>
{
    public BlackyGenericPalette<SuperficieData> pallete { get; } = new("Superficie");

    private DualTileTemplate _dualTemplate;
    public BlackySurfaceWorldData(int chunkSize, BlackyChunkCacheTextureMap textureMap, BlackyWorldRegions regions) : base(chunkSize, BlackyRenderLayer.Superficie, textureMap, true, regions)
    {
    ...
    public void SetSuperficie(int worldX, int worldY, int height, string modName, long terrainId, Brush brush)
    {
        ...
        foreach (var offset in brush.Cells)
        {
            ...
            cell = pallete.GetIdPersistence(modName, terrainId, out data);
        }
    ...
    public void RemoveSuperficie(int worldX, int worldY, int height, Brush brush)
    {
