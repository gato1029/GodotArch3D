...
public class BlackyPathWorldData: BlackyWorldDataMap<SerializerCellGeneric>
{
    ...
    public BlackyPathWorldData(int chunkSize, BlackyChunkCacheTextureMap textureMap, BlackyWorldRegions regions, ChunkManagerBase chunkManager, BlackyWorld blackyWorld)
        : base(chunkSize, BlackyRenderLayer.Caminos, textureMap, true, regions, blackyWorld )
    {
    ...
    public void SetCaminoDirectNoRender(int x, int y, int h, bool isBorder, SuperficieData data)
    {
    ...
    public void RemoveCaminoDirectNoRender(int x, int y, int h)
    {
        ref var cell = ref ResolveOrCreateCell(x, y, h);
        cell.id = 0;
        cell.isBorder = false;
    ...
    public void SetCamino(int x, int y, int h, CaminosData data, Brush brush)
    {
        ...
        foreach (var offset in brush.Cells)
        {
            ...
            cell.id = BlackyPalletesPersistence.pathsPalette.GetIdPersistence(data.nameMod, data.id, out _);
        }
    ...
    public void RemoveCamino(int x, int y, int h, Brush brush)
    {
        ...
        UpdateBorderStatesForBrush(x, y, h, brush);
        var data = BlackyPalletesPersistence.pathsPalette.GetData(lastId);
        _dualTemplate = AtlasModsManager.Get<DualTileTemplate>(data.nameMod, data.idDualTemplate);
        ...
        for (int oy = -1; oy <= 1; oy++)
        {
            for (int ox = -1; ox <= 1; ox++)
            {
                foreach (var offset in brush.Cells)
                {
                    ...
                    // Si HasCamino devuelve true, significa que la celda existe y tiene ID != 0
                    if (HasCamino(x, y, h))
                    {
    ...
    public bool HasCamino(int worldX, int worldY, int height)
    {
    ...
    private bool CalculateIsBorder(int x, int y, int h)
    {
        for (int oy = -1; oy <= 1; oy++)
        {
            for (int ox = -1; ox <= 1; ox++)
            {
                ...
                if (!HasCamino(x + ox, y + oy, h)) return true;
            }
        }
        return false;
    }
